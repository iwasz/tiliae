...
namespace {
        ...
        void Impl::onCloseElement (mxml_node_t *node)
        {
                ...
                else if (!strcmp (name, "beans") || !strcmp (name, "null") || !strcmp (name, "ref") || !strcmp (name, "import") || !strcmp (name, "prop-alias")
                         || !strcmp (name, "cargs")) {
                        // Ignore
                }
                else {
        ...
        bool Impl::checkIfClass (std::string const &name) const { return static_cast<bool> (Reflection::Manager::classForName (name)); }

        /****************************************************************************/

        bool Impl::checkIfSpecial (std::string const &name) const { return name == "bean" || name == "map" || name == "list"; }

        /****************************************************************************/

        void Impl::validateId (std::string const &name) const
        {
                if (name == "bean" || name == "map" || name == "list" || name == "value" || name == "ref" || name == "beans" || name == "null"
                    || name == "cargs") {
...
void Impl::mxmlErrorCallback (const char *s) { throw XmlMetaServiceException ("CompactMetaService::parseFile : XML has errors [" + std::string (s) + "]"); }
...
Ptr<MetaContainer> CompactMetaService::parseFile (Common::DataSource *ds, std::string const &path, Ptr<MetaContainer> container)
{
        ...
        mxmlSAXLoadString (nullptr, xml.c_str (), MXML_OPAQUE_CALLBACK, saxHandler, &impl);
