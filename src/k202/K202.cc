...
Ptr<K202> K202::create (Ptr<Wrapper::IBeanWrapper> b, Ptr<IExtension> ext)
{
        ...
        k->ctx.setBeanWrapper (std::unique_ptr<IBeanWrapper> (BeanWrapper::create ()));
...
template <typename T> T K202::createHelper (Ptr<IExtension> ext)
{
        T k (new K202);
        k->ctx.setBeanWrapper (std::unique_ptr<IBeanWrapper> (BeanWrapper::create ()));
        k->compiler = Compiler::create ();
...
Core::Variant K202::run (const std::string &sourceCode, const Core::Variant &domain, const Core::VariantVector &paramVector, const Core::VariantMap &argsMap)
{
...
Core::Variant K202::run (Ptr<Script> script, Core::Variant *domain, const Core::VariantVector &paramVector, const Core::VariantMap &argsMap)
{
        /*
...
Ptr<Script> K202::prepare (const std::string &sourceCode, const Core::Variant &domain, const Core::VariantVector &paramVector, const Core::VariantMap &argsMap)
{
